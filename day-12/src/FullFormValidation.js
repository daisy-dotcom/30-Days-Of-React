...
class App extends Component {
    ...
    validate = () => {
        ...
        if (
            ...
            (this.state.touched.email && this.state.email.length > 50)
        ){
            errors.email = 'Email must be between 2 and 50'
        }

        if (
            this.state.touched.country && this.state.email.length === 0
        ){
            errors.country = 'Select a country'
        }

        if (
            this.state.touched.gender && this.state.gender.length === 0
        ){
            errors.gender = 'Select a gender'
        }

        if (
            this.state.touched.tel && this.state.tel.length < 10
        ){
            errors.tel = 'Enter a phone number with 10 digits'
        }

        if (
            this.state.touched.dateOfBirth && this.state.dateOfBirth.length < 10
        ){
            errors.dateOfBirth = 'Enter a DOB'
        }

        if (
            this.state.touched.favoriteColor && this.state.favoriteColor.length < 7
        ){
            errors.favoriteColor = 'Select a colour'
        }

        if (
            (this.state.touched.weight && this.state.weight < 50) ||
            (this.state.touched.weight && this.state.weight > 100)
        ){
        ...
        if (
            this.state.touched.bio && this.state.bio.length === 0
        ){
            errors.bio = 'Enter a short biography'
        }

        if (
            this.state.touched.file && this.state.file.length === 0
        ){
